...
server <- function(input, output, session) {
  ...
  observeEvent(input$file, {
    ...
    hist_plot <- reactive({
      ggplot(
        df() |> mutate(distribution = factor(distribution, levels = rev(input$dist))),
    ...
    line_plot <- reactive({
      ggplot(
        df() |> mutate(distribution = factor(distribution, levels = input$dist)),
        aes(x = n, y = data),
      ) +
        geom_line() +
        geom_point() +
    ...
    tbl <- reactive({
      reactable(
        ...
        columns = list(
          ...
          `Std Dev` = colDef(align = "center", format = colFormat(digits = 2)),
